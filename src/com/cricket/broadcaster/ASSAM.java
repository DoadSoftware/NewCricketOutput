...
public class ASSAM extends Scene{

	private String status;
	private String slashOrDash = "-";
	public Infobar infobar = new Infobar();
	public String session_selected_broadcaster = "ASSAM";
	public String which_graphics_onscreen = "";
	private String logo_path = "C:\\Images\\ASSAM\\Logos\\";
	private String photo_path = "C:\\Images\\ASSAM\\Photos\\";
	private String local_photo_path = "\\\\c\\\\Images\\\\ASSAM\\\\Photos\\\\";
	
	public ASSAM() {
		super();
	}

	public ASSAM(String scene_path, String which_Layer) {
	...
	public Object ProcessGraphicOption(String whatToProcess, MatchAllData match, CricketService cricketService, List<MatchAllData> tournament_matches,
		...
		case "THIS_SESSION_GRAPHICS-OPTIONS": case "FF_EQUATION_GRAPHICS-OPTIONS": case "L3PLAYERPROFILEBUKHATIR_GRAPHICS-OPTIONS": case "PLAYERPROFILEASSAM_GRAPHICS-OPTIONS":
			switch (session_selected_broadcaster.toUpperCase()) {
			case "ASSAM": case "DOAD_IN_HOUSE_VIZ":
		...
		case "POPULATE-FF-EQUATION": case "POPULATE-L3-BUG-TOSS": case "POPULATE-L3-PLAYERPROFILE_BUKHATIR": case "POPULATE-FF-PLAYERPROFILE_ASSAM":
			switch (session_selected_broadcaster.toUpperCase()) {
			case "ASSAM":
				switch(whatToProcess.toUpperCase()) {
				...
				case "POPULATE-FF-MATCHSUMMARY":
					populateMatchsummary(print_writer, valueToProcess.split(",")[0], Integer.valueOf(valueToProcess.split(",")[1]),match, session_selected_broadcaster);
					break;
				...
				case "POPULATE-FF-PLAYERPROFILE":
					...
					break;
				case "POPULATE-FF-PLAYERPROFILE_ASSAM":
					for(Statistics stats : cricketService.getAllStats()) {
						if(stats.getPlayer_id() == Integer.valueOf(valueToProcess.split(",")[1])) {
							stats.setStats_type(cricketService.getStatsType(stats.getStats_type_id()));
							stats = CricketFunctions.updateTournamentDataWithStats(stats, tournament_matches, match, CricketUtil.FULL);
...
//								new File(CricketUtil.CRICKET_SERVER_DIRECTORY + CricketUtil.MATCHES_DIRECTORY + file.getName()))));
			...
			case "ASSAM":
				...
				case "ANIMATE-IN-SCORECARD":
					...
					if(match.getMatch().getInning().get(0).getBattingCard().size() == 12) {
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main$BattingCard_GRP$DataAll$BAtting$Batting_Card$12*CONTAINER SET ACTIVE 1;");
						processAnimation(print_writer, "CON1In", "START", session_selected_broadcaster,1);
					}
					...
					if(match.getMatch().getInning().get(0).getBattingCard().size() == 13) {
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main$BattingCard_GRP$DataAll$BAtting$Batting_Card$12*CONTAINER SET ACTIVE 1;");
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main$BattingCard_GRP$DataAll$BAtting$Batting_Card$13*CONTAINER SET ACTIVE 1;");
						processAnimation(print_writer, "CON2In", "START", session_selected_broadcaster,1);
					}
					...
					processAnimation(print_writer, "BattingCardIn", "START", session_selected_broadcaster,1);
				...
				case "ANIMATE-IN-BOWLINGCARD":
					processAnimation(print_writer, "In", "START", session_selected_broadcaster,1);
					processAnimation(print_writer, "BowlingCardIn", "START", session_selected_broadcaster,1);
					print_writer.println("LAYER1*EVEREST*STAGE*DIRECTOR*LOOP START;");
					//print_writer.println("LAYER2*EVEREST*STAGE START;");
				...
				case "ANIMATE-IN-PARTNERSHIP":
					if(match.getMatch().getInning().get(0).getBattingCard().size() == 12) {
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main$12*CONTAINER SET ACTIVE 1;");
						processAnimation(print_writer, "CON1In", "START", session_selected_broadcaster,1);
					}
					else if(match.getMatch().getInning().get(1).getBattingCard().size() == 12) {
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main$12*CONTAINER SET ACTIVE 1;");
					...
					if(match.getMatch().getInning().get(0).getBattingCard().size() == 13) {
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main$13*CONTAINER SET ACTIVE 1;");
						processAnimation(print_writer, "CON2In", "START", session_selected_broadcaster,1);
					}
					else if(match.getMatch().getInning().get(1).getBattingCard().size() == 13) {
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main$13*CONTAINER SET ACTIVE 1;");
				...
				case "ANIMATE-IN-MATCHSUMARRY":
					...
					processAnimation(print_writer, "In", "START", session_selected_broadcaster,1);
					print_writer.println("LAYER1*EVEREST*STAGE*DIRECTOR*LOOP START;");
					processAnimation(print_writer, "SummaryIn", "START", session_selected_broadcaster,1);
				...
				case "ANIMATE-IN-POINTSTABLE":
					processAnimation(print_writer, "In", "START", session_selected_broadcaster,1);
					processAnimation(print_writer, "PointsTableIn", "START", session_selected_broadcaster,1);
					print_writer.println("LAYER1*EVEREST*STAGE START;");
				...
				case "ANIMATE-OUT":
					...
					case "SUMARRY": case "PREVIOUS_SUMARRY":
						...
						break;
					case "POINTSTABLE":
						processAnimation(print_writer, "PointsTableOut", "START", session_selected_broadcaster,1);
						TimeUnit.SECONDS.sleep(1);
			...
			if (match == null) {
				...
				for(Inning inn : match.getMatch().getInning()) {
					if (inn.getInningNumber() == whichInning) {
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTeamFirstName " + inn.getBatting_team().getTeamName4().toUpperCase() + ";");
						print_writer.println("LAYER1*EVEREST*TREEVIEW*LastName*CONTAINER SET ACTIVE 0;");
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSubHeader01 " + "v " + inn.getBowling_team().getTeamName4().toUpperCase() + ";");
						...
						//inn.getBattingCard().get(0).getPlayer().getTicker_name();
						...
						for (BattingCard bc : inn.getBattingCard()) {
							...
							case CricketUtil.OUT:
								...
								if(bc.getHowOut().toUpperCase().equalsIgnoreCase(CricketUtil.CONCUSSED)) {
									...
									print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tPlayerName0" +row_id + " " +bc.getPlayer().getTicker_name().toUpperCase() + ";");
									...
									if(bc.getWasHowOutFielderSubstitute() != null && bc.getWasHowOutFielderSubstitute().equalsIgnoreCase(CricketUtil.YES)) {
										print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tHowOut0"+ row_id + "A" + " " + bc.getHowOutPartOne() + " (SUB)" + ";");
									}else {
										print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tHowOut0"+ row_id + "A" + " " + bc.getHowOutPartOne() + ";");
									}
							...
							case CricketUtil.STILL_TO_BAT:
								...
								if(bc.getHowOut() == null) {
									...
									print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tPlayerName0" +row_id + " " +bc.getPlayer().getTicker_name().toUpperCase() + ";");
									print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET vHighlight_DeHighlight0" + row_id + " " + "1" + ";");
									print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tHowOut0"+ row_id + "A" + " " + "RETIRED HURT" + ";");
									print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tHowOut0"+ row_id + "B" + " " + " " + ";");
									print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tRun0" + row_id + " " + bc.getRuns() + ";");
									print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBalls0" + row_id + " " + bc.getBalls() + ";");
								}
				...
				print_writer.println("LAYER1*EVEREST*STAGE*DIRECTOR*In SHOW 141.0;");
		...
		case "ASSAM":
			if (match == null) {
				...
				for(Inning inn : match.getMatch().getInning()) {
					if (inn.getInningNumber() == whichInning) {
						...
						if (inn.getBowlingTeamId() == match.getSetup().getHomeTeamId()) {
							//commands = commands + "LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTeamFirstName " + match.getHomeTeam().getFullname().toUpperCase() + ";" ;
							//commands = commands + "LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSubHeader01 " + "v " + match.getAwayTeam().getFullname().toUpperCase() + ";" ;
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTeamFirstName " + match.getSetup().getHomeTeam().getTeamName4().toUpperCase() + ";");
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSubHeader01 " + "v " + match.getSetup().getAwayTeam().getTeamName4().toUpperCase() + ";");
						} else {
							//commands = commands + "LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTeamFirstName " + match.getAwayTeam().getFullname().toUpperCase() + ";" ;
							//commands = commands + "LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSubHeader01 " + "v " + match.getHomeTeam().getFullname().toUpperCase() + ";" ;
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTeamFirstName " + match.getSetup().getAwayTeam().getTeamName4().toUpperCase() + ";");
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSubHeader01 " + "v " + match.getSetup().getHomeTeam().getTeamName4().toUpperCase() + ";");
						}
						//commands = commands + "LAYER1*EVEREST*TREEVIEW*Main$Data$Top$Logo$TeamLogo*CONTAINER SET ACTIVE 1;" ;
						//commands = commands + "LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgTeamLogo " + logo_path + inn.getBowling_team().getFullname() + CricketUtil.PNG_EXTENSION + ";" ;
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main$Data$Top$Logo$TeamLogo*CONTAINER SET ACTIVE 1;");
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgTeamLogo " + logo_path + inn.getBowling_team().getTeamName4() + CricketUtil.PNG_EXTENSION + ";");
						...
						for (BowlingCard boc : inn.getBowlingCard()) {
							...
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tPlayerName0" + row_id + " "  + boc.getPlayer().getTicker_name().toUpperCase() + ";");
							...
							if(boc.getEconomyRate() == null) {
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatValue" + row + "E " + "-" + ";");
							}else {
			    ...
			    try {  
			        ...
			        fh = new FileHandler("C:/Temp/MyLogFile.txt");  
			...
			if (match == null) {
				...
				for(Inning inn : match.getMatch().getInning()) {
					if(inn.getInningNumber() == whichInning) {
						...
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgTeamLogo " + logo_path + inn.getBatting_team().getTeamName4() + 
						...
						for(Partnership ps : inn.getPartnerships()) {
							...
							/*if(inn.getBattingCard().size() == 12) {
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main$DataAll$BAtting$Batting_Card$12*CONTAINER SET ACTIVE 1;");
							}
							else if(inn.getBattingCard().size() == 13) {
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main$DataAll$BAtting$Batting_Card$13*CONTAINER SET ACTIVE 1;");
							}*/
							
							ScaleFac1 = ((ps.getFirstBatterRuns())*(Mult/Top_Score)) ;
							ScaleFac2 = ((ps.getSecondBatterRuns())*(Mult/Top_Score)) ;
							
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET vSelectionOmo" + row_id + " " + omo_num + ";");
							
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tLeftPlayerName" + row_id + " " + Left_Batsman + ";");
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tRightPlayerName" + row_id + " " + Right_Batsman + ";");
							
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tLeftBar" + row_id + " " + ScaleFac1 + ";");
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tRightBar" + row_id + " " + ScaleFac2 + ";");
							
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tPartnershipRun" + row_id + " " + ps.getTotalRuns() + ";");
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tPartnershipBall" + row_id + " " + ps.getTotalBalls() + ";");
								
						}
						
						if(inn.getPartnerships().size() >= 10) {
							row_id = row_id + 1;
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET vSelectionOmo" + row_id + " " + "4" + ";");
						}
						else {
							for (BattingCard bc : inn.getBattingCard()) {
								if(row_id < inn.getBattingCard().size()) {
									if(row_id == inn.getPartnerships().size()) {
										row_id = row_id + 1;
										print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET vSelectionOmo" + row_id + " " + "0" + ";");
										print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tDidNotBat" + row_id + " " + "DID NOT BAT" + ";");
									}
									else if(bc.getStatus().toUpperCase().equalsIgnoreCase(CricketUtil.STILL_TO_BAT)) {
										row_id = row_id + 1;
										print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET vSelectionOmo" + row_id + " " + "1" + ";");
										print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tLeftPlayerName" + row_id + " " + bc.getPlayer().getTicker_name().toUpperCase() + ";");
									}	
								}
								else {
									break;
	...
	public void populateMatchsummary(PrintWriter print_writer, String viz_scene, int whichInning, MatchAllData match, String session_selected_broadcaster) throws InterruptedException
	{
		...
		case "ASSAM":
			if (match == null) {
				...
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTeamFirstName " + match.getSetup().getMatchIdent().toUpperCase() +  ";");
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTeamLastName " + " SUMMARY" + ";");
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSubHeader01 " + match.getSetup().getHomeTeam().getTeamName1().toUpperCase() + 
						" vs " + match.getSetup().getAwayTeam().getTeamName1().toUpperCase() + ";");
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgTeamLogo " + logo_path + "TLogo" + ".png" + ";");
				...
				for(int i = 1; i <= whichInning ; i++) {
					...
					if(match.getMatch().getInning().get(i-1).getTotalWickets() >= 10) {
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tScore0"+ row + " " + match.getMatch().getInning().get(i-1).getTotalRuns() + ";");
					}
					else {
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tScore0"+ row + " " + match.getMatch().getInning().get(i-1).getTotalRuns() 
					...
					if(match.getMatch().getInning().get(i-1).getBattingCard() != null) {
						...
						for(BattingCard bc : match.getMatch().getInning().get(i-1).getBattingCard()) {
							if(bc.getBatsmanInningStarted() != null && bc.getBatsmanInningStarted().equalsIgnoreCase(CricketUtil.YES) && bc.getStatus().equalsIgnoreCase(CricketUtil.STILL_TO_BAT)) {
								...
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBatsmanName0"+ row_id + " " + bc.getPlayer().getTicker_name().toUpperCase() + " (rh)" + ";");
							...
							if(!bc.getStatus().toUpperCase().equalsIgnoreCase(CricketUtil.STILL_TO_BAT)) {
								row_id = row_id + 1;
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Batsman0"+ row_id + "*CONTAINER SET ACTIVE 1;");
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBatsmanName0"+ row_id + " " + bc.getPlayer().getTicker_name().toUpperCase() + ";");
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBatsmanRuns0"+ row_id + " " + bc.getRuns() + ";");
					...
					if(match.getMatch().getInning().get(i-1).getBowlingCard() != null) {
						...
						for(BowlingCard boc : match.getMatch().getInning().get(i-1).getBowlingCard()) {
							if(boc.getWickets() > 0 ) {
								...
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBowlerName0" + row_id + " " +  boc.getPlayer().getTicker_name().toUpperCase() + ";");
				...
				if(match.getMatch().getMatchResult() != null) {
					if(match.getMatch().getMatchResult().toUpperCase().equalsIgnoreCase("DRAWN")){
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tResult " + "MATCH TIED" + ";");
					}
					else if(match.getMatch().getMatchResult().split(",")[1].toUpperCase().equalsIgnoreCase("SUPER_OVER")){
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tResult " + "MATCH TIED - " + match.getMatch().getMatchStatus().toUpperCase() + ";");
					}
					else {
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tResult " + 
	...
	public void populateTeamsLogo(PrintWriter print_writer,String viz_scene, List<Team> teams ,MatchAllData match, String session_selected_broadcaster) throws InterruptedException {
		...
		print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tHeader " + "TEAMS" +";");
		print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSubHeader " + match.getSetup().getTournament().toUpperCase() + ";");
		...
		print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgHomeTeamLogo " + logo_path + teams.get(0).getTeamName1() + CricketUtil.PNG_EXTENSION + ";");
		print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgAwayTeamLogo " + logo_path + teams.get(1).getTeamName1() + CricketUtil.PNG_EXTENSION + ";");
		print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgAwayTeam1Logo " + logo_path + teams.get(2).getTeamName1() + CricketUtil.PNG_EXTENSION + ";");
		print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgHomeTeam1Logo " + logo_path + teams.get(3).getTeamName1() + CricketUtil.PNG_EXTENSION + ";");
//		print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgAwayTeam2Logo " + logo_path + teams.get(5).getTeamName1() + CricketUtil.PNG_EXTENSION + ";");
//		print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgAwayTeam3Logo " + logo_path + teams.get(6).getTeamName1() + CricketUtil.PNG_EXTENSION + ";");
		...
		case "ASSAM":
			if (match == null) {
				...
				if(match.getSetup().getTossWinningTeam() == match.getSetup().getHomeTeamId()) {
					...
					print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tInfo1A " + " WON THE TOSS & ELECTED TO " + match.getSetup().getTossWinningDecision() + ";");
						...
						case CricketUtil.BATSMAN :
							for (BattingCard bc : inn.getBattingCard()) {
								if(bc.getPlayerId()==playerId) {
									print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tPlayerName01 " + bc.getPlayer().getFirstname().toUpperCase() + ";");
						...
						case CricketUtil.BOWLER :
							for (BowlingCard boc : inn.getBowlingCard()) {
								if(boc.getPlayerId()==playerId) {
									print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tPlayerName01 " + boc.getPlayer().getFirstname().toUpperCase() + ";");
				...
				for(Inning inn : match.getMatch().getInning()) {
					if (inn.getInningNumber() == whichInning) {
						for (BattingCard bc : inn.getBattingCard()) {
							if(bc.getPlayerId()==playerId) {
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgTeamLogo " + logo_path + inn.getBatting_team().getTeamName4() + CricketUtil.PNG_EXTENSION  + ";");
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFirstName " + bc.getPlayer().getFull_name().toUpperCase() + ";");
								print_writer.println("LAYER1*EVEREST*TREEVIEW*LastName*CONTAINER SET ACTIVE 0;");
					...
					if (inn.getInningNumber() == whichInning) {
						for (BattingCard bc : inn.getBattingCard()) {
							if(bc.getPlayerId()==playerId) {
								...
								print_writer.println("LAYER1*EVEREST*TREEVIEW*LastName*CONTAINER SET ACTIVE 0;");
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tRuns " + bc.getRuns() + ";");
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBalls " + ( bc.getBalls() + 1 ) + ";");
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tHowOut " + "FOURS " + bc.getFours() + ";");
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBowler " + "SIXES " + bc.getSixes() + ";");
								
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatHead01 " + " " + ";");
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatHead02 " + " " + ";");
								
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatValue01 " + " " + ";");
				...
				for(Inning inn : match.getMatch().getInning()) {
					if (inn.getInningNumber() == whichInning) {
						...
						case CricketUtil.BATSMAN :
							for (BattingCard bc : inn.getBattingCard()) {
								if(bc.getPlayerId()==playerId) {
									print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tPlayerName " + bc.getPlayer().getFull_name().toUpperCase() + ";");
									...
									if (inn.getBattingTeamId() == match.getSetup().getHomeTeamId()) {
										print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgTeamLogo " + logo_path + match.getSetup().getHomeTeam().getTeamName4() + CricketUtil.PNG_EXTENSION + ";");
									} else {
										print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgTeamLogo " + logo_path + match.getSetup().getAwayTeam().getTeamName4() + CricketUtil.PNG_EXTENSION + ";");
									}
				...
				for(Inning inn : match.getMatch().getInning()) {
					if (inn.getInningNumber() == whichInning) {
						...
						case CricketUtil.BOWLER:
							for (BowlingCard boc : inn.getBowlingCard()) {
								if(boc.getPlayerId()==playerId) {
									print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgTeamLogo " + logo_path + team.get(boc.getPlayer().
											getTeamId() - 1).getTeamName4() + CricketUtil.PNG_EXTENSION + ";");
									print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tPlayerName " + boc.getPlayer().getFull_name().toUpperCase() + ";");
									
									print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateHead " + "OVERS" + ";");
									print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateValue " + CricketFunctions.OverBalls(boc.getOvers(), boc.getBalls()) + ";");
									...
									print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateHead4 " + "ECON" + ";");
		...
		case "ASSAM":
			if (match == null) {
				...
				if(ns.getFirstname() == null) {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFirstName " + ns.getSurname().toUpperCase() +";");
				}
				else if(ns.getSurname() == null) {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFirstName " + ns.getFirstname().toUpperCase() +";");
				}
				else {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFirstName " + ns.getFirstname().toUpperCase() + " " 
				...
				print_writer.println("LAYER2*EVEREST*STAGE*DIRECTOR*In STOP;");
				print_writer.println("LAYER2*EVEREST*STAGE*DIRECTOR*Out STOP;");
				print_writer.println("LAYER2*EVEREST*STAGE*DIRECTOR*In SHOW 39.0;");
				print_writer.println("LAYER2*EVEREST*STAGE*DIRECTOR*Out SHOW 0.0;");
				print_writer.println("LAYER1*EVEREST*GLOBAL SNAPSHOT_PATH C:/Temp/Preview.bmp;");
				print_writer.println("LAYER1*EVEREST*GLOBAL SNAPSHOT 1920 1080;");
				TimeUnit.SECONDS.sleep(1);
		...
		case "ASSAM":
			if (match == null) {
				...
				if(TeamId == match.getSetup().getHomeTeamId()) {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgTeamLogo " + logo_path + match.getSetup().getHomeTeam().getTeamName4() + 
							CricketUtil.PNG_EXTENSION + ";");
					Home_or_Away = match.getSetup().getHomeTeam().getTeamName4().toUpperCase();
					for(Player hs : match.getSetup().getHomeSquad()) {
						if(playerId == hs.getPlayerId()) {
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFirstName " + hs.getFull_name().toUpperCase() +";");
							...
							if(captainWicketKeeper.toUpperCase().equalsIgnoreCase(CricketUtil.PLAYER)) {
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tDetails " + match.getSetup().getHomeTeam().getTeamName4().toUpperCase() + ";");
							}
						}
					...
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgTeamLogo " + logo_path + match.getSetup().getAwayTeam().getTeamName4() + 
							CricketUtil.PNG_EXTENSION + ";");
					Home_or_Away = match.getSetup().getAwayTeam().getTeamName4().toUpperCase();
					for(Player as : match.getSetup().getAwaySquad()) {
						if(playerId == as.getPlayerId()) {
							...
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFirstName " + as.getFull_name().toUpperCase() +";");
							...
							if(captainWicketKeeper.toUpperCase().equalsIgnoreCase(CricketUtil.PLAYER)) {
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tDetails " + match.getSetup().getAwayTeam().getTeamName4().toUpperCase() + ";");
							}
						}
					}
				}
				...
				case CricketUtil.CAPTAIN:
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tDetails " + captainWicketKeeper.toUpperCase() + " , " + Home_or_Away + ";");
					break;
				case "PLAYER OF THE MATCH":
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tDetails " + captainWicketKeeper.toUpperCase() + ";");
					break;
				case CricketUtil.WICKET_KEEPER:
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tDetails " + "WICKET KEEPER" + " , " + Home_or_Away + ";");
					break;
				...
				case "CAPTAIN-WICKETKEEPER":
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tDetails " + "CAPTAIN & WICKET KEEPER" + " , " + Home_or_Away + ";");
					break;
				}
		...
		case "ASSAM":
			...
			if(plyr.getTeamId() == match.getSetup().getHomeTeamId()) {
				if(config.getPrimaryIpAddress().equalsIgnoreCase("LOCALHOST")) {
					...
					if(!new File("\\\\"+config.getPrimaryIpAddress()+"\\c\\Images\\ASSAM\\Photos\\" +match.getSetup().getHomeTeam().getTeamName4()
							+ "\\\\" + plyr.getPhoto() + CricketUtil.PNG_EXTENSION).exists()) {
						this.status = CricketUtil.UNSUCCESSFUL;
					}
					print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgPlayerImage01 " + "\\\\\\\\"+config.getPrimaryIpAddress()+"\\c\\Images\\ASSAM\\Photos\\" + match.getSetup().getHomeTeam().getTeamName4()
							+ "\\\\" + plyr.getPhoto() + CricketUtil.PNG_EXTENSION + ";");
				}
				...
				if(config.getPrimaryIpAddress().equalsIgnoreCase("LOCALHOST")) {
					...
					if(!new File("\\\\"+config.getPrimaryIpAddress()+"\\c\\Images\\ASSAM\\Photos\\" +match.getSetup().getAwayTeam().getTeamName4()
							+ "\\\\" + plyr.getPhoto() + CricketUtil.PNG_EXTENSION).exists()) {
						this.status = CricketUtil.UNSUCCESSFUL;
					}
					print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgPlayerImage01 " + "\\\\\\\\"+config.getPrimaryIpAddress()+"\\\\"+local_photo_path + match.getSetup().getAwayTeam().getTeamName4()
							+ "\\\\" + plyr.getPhoto() + CricketUtil.PNG_EXTENSION + ";");
				}
				print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFirstName " + plyr.getFirstname() + ";");
				print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tLastName " + plyr.getSurname() + ";");
	...
	public void populateLTPlayerProfile(PrintWriter print_writer,String viz_scene,String Profile,String TypeofProfile,Statistics stats, MatchAllData match, String session_selected_broadcaster) throws InterruptedException 
	{
		...
		case "ASSAM":
			...
			Player plyr = getPlayerFromMatchData(stats.getPlayer_id(), match);
			if(plyr.getTeamId() == match.getSetup().getHomeTeamId()) {
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgTeamLogo " + logo_path + match.getSetup().getHomeTeam().getTeamName1() + CricketUtil.PNG_EXTENSION + ";");
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tPlayerName " + plyr.getFull_name().toUpperCase() + ";");
			}
			else {
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgTeamLogo " + logo_path + match.getSetup().getAwayTeam().getTeamName1() + CricketUtil.PNG_EXTENSION + ";");
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tPlayerName " + plyr.getFull_name().toUpperCase() + ";");
			}
			switch(TypeofProfile.toUpperCase()) {
			case CricketUtil.BATSMAN:
				...
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateHead1 " + "INNINGS" +";");
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateValue1 " + stats.getInnings() + ";");
				
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateHead2 " + "RUNS" +";");
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateValue2 " + stats.getRuns() +";");
				
				strike_rate = stats.getRuns() * 100;
				strike_rate = strike_rate/stats.getBalls_faced();
				DecimalFormat df = new DecimalFormat("0.0");
				
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateHead3 " + "S/R" + ";");
				if(stats.getRuns()== 0 && stats.getBalls_faced() == 0) {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateValue3 " + "-" +";");
				}else {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateValue3 " + df.format(strike_rate) +";");
				}
				
				/*if(stats.getThirties() == null &&  stats.getFifties() == null) {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateHead4 " + "30s/50s" + ";");
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateValue4 " + "0/0"+";");
				}else {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateHead4 " + "30s/50s" + ";");
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateValue4 " + stats.getThirties()+"/"+stats.getFifties()+";");
				}*/
				
				if(stats.getBest_score() == null) {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateHead4 " + "BEST" + ";");
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateValue4 " + "-"+";");
				}else {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateHead4 " + "BEST" + ";");
					if(Integer.valueOf(stats.getBest_score()) % 2 > 0) {
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateValue4 " + (Integer.valueOf(stats.getBest_score()) - 1)/2 + "*" +";");
					}else {
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateValue4 " + (Integer.valueOf(stats.getBest_score())/2) +";");
					}
				}
			...
			case CricketUtil.BOWLER:
				...
				//print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateHead2 "+ "DOTS"+";");
				//print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateValue2 "+ stats.getDotbowled() +";");
				
				economy_rate = stats.getRuns_conceded() / stats.getBalls_bowled();
				DecimalFormat df_b = new DecimalFormat("0.00");
				
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateHead2 "+"ECON."+";");
				if(stats.getRuns_conceded() == 0 && stats.getBalls_bowled() == 0) {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateValue2 "+ "-" +";");
				}else {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateValue2 "+ df_b.format(economy_rate) +";");
				}
				
				
				if(stats.getPlus_3() == null &&  stats.getPlus_5() == null) {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateHead3 " + "3WI / 5WI" + ";");
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateValue3 " + "0/0"+";");
				}else {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateHead3 " + "3WI / 5WI" + ";");
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateValue3 " + stats.getPlus_3()+"/"+stats.getPlus_5()+";");
				}
				...
				if(stats.getBest_figures() == null) {
					...
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateHead4 "+ "BEST" +";");
					if(Integer.valueOf(stats.getBest_figures()) % 1000 > 0) {
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateValue4 "+ ((Integer.valueOf(stats.getBest_figures()) / 1000)+1) + 
								"-" + (1000 - (Integer.valueOf(stats.getBest_figures()) % 1000)) +";");
					} else {
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateValue4 "+ (Integer.valueOf(stats.getBest_figures()) / 1000) + 
								"-" + (1000 - (Integer.valueOf(stats.getBest_figures()) % 1000)) +";");
					}
				}
				break;
			}
			print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSubHeader " + Profile.toUpperCase() + ";");
	...
	public void populateDoubleteams(PrintWriter print_writer,String viz_scene, MatchAllData match, String session_selected_broadcaster) throws InterruptedException
	{
		...
		case "ASSAM":
			if (match == null) {
				...
				for(int i = 1; i <= 2 ; i++) {
					if(i == 1) {
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTeamFirstName " + "TEAMS - " + match.getSetup().getMatchIdent().toUpperCase() + ";");
						if(match.getSetup().getTossWinningTeam() == match.getSetup().getHomeTeamId()) {
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSubHeader01 " + match.getSetup().getHomeTeam().getTeamName4().toUpperCase() + 
									" WON THE TOSS & ELECTED TO " + match.getSetup().getTossWinningDecision() + ";");
						}else {
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSubHeader01 " + match.getSetup().getAwayTeam().getTeamName4().toUpperCase() + 
									" WON THE TOSS & ELECTED TO " + match.getSetup().getTossWinningDecision() + ";");
						}
						
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tHomeTeamName " + match.getSetup().getHomeTeam().getTeamName4().toUpperCase() + ";");
						...
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tAwayTeamName " + match.getSetup().getAwayTeam().getTeamName4().toUpperCase() + ";");
						for(Player as : match.getSetup().getAwaySquad()) {
							row_id = row_id + 1;
				...
				print_writer.println("LAYER1*EVEREST*STAGE*DIRECTOR*In STOP;");
				print_writer.println("LAYER1*EVEREST*STAGE*DIRECTOR*Out STOP;");
				print_writer.println("LAYER1*EVEREST*STAGE*DIRECTOR*In SHOW 134.0;");
				print_writer.println("LAYER1*EVEREST*STAGE*DIRECTOR*Out SHOW 0.0;");
				print_writer.println("LAYER1*EVEREST*GLOBAL SNAPSHOT_PATH C:/Temp/Preview.bmp;");
				print_writer.println("LAYER1*EVEREST*GLOBAL SNAPSHOT 1920 1080;");
				TimeUnit.SECONDS.sleep(1);
			...
			for(Inning inn : match.getMatch().getInning()) {
				if (inn.getIsCurrentInning().toUpperCase().equalsIgnoreCase(CricketUtil.YES)) {
					if(is_this_updating == false) {
						...
						//processAnimation(print_writer, "Sponsor_In", "START", session_selected_broadcaster,1);
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgBatTeamLogo " + logo_path + "Round\\" + inn.getBatting_team().getTeamName4() + CricketUtil.PNG_EXTENSION + ";");
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgBallTeamLogo " + logo_path + "Round\\" + inn.getBowling_team().getTeamName4() + CricketUtil.PNG_EXTENSION + ";");
					}
					...
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBatTeamName " + inn.getBatting_team().getTeamName4().toUpperCase() + ";");
					if(inn.getTotalWickets() >= 10) {
		...
		for(Inning inn : match.getMatch().getInning()) {
			...
			if (inn.getIsCurrentInning().toUpperCase().equalsIgnoreCase(CricketUtil.YES)) {
				...
				if(current_batsmen != null && current_batsmen.size() >= 2) {
					...
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBatsmanName1 " + current_batsmen.get(0).getPlayer().getTicker_name() + ";");
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBatsmanScore1 " + current_batsmen.get(0).getRuns() + ";");
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBatsmanBall1 "+ current_batsmen.get(0).getBalls() + ";");
					
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBatsmanName2 " + current_batsmen.get(1).getPlayer().getTicker_name() + ";");
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBatsmanScore2 " + current_batsmen.get(1).getRuns() + ";");
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBatsmanBall2 "+ current_batsmen.get(1).getBalls() + ";");
				...
				if (inn.getIsCurrentInning().toUpperCase().equalsIgnoreCase(CricketUtil.YES)) {
					for(BowlingCard boc : inn.getBowlingCard()) {
						if(boc.getStatus().toUpperCase().equalsIgnoreCase("CURRENTBOWLER") 
							...
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBowlerName " + boc.getPlayer().getTicker_name() + ";");
	...
	public Infobar populateInfobarBottomLeft(Infobar infobar,boolean is_this_updating, PrintWriter print_writer, MatchAllData match, String session_selected_broadcaster)
	{
		...
		case "ASSAM":
			...
			case "WHICH_INNING":
				if(is_this_updating == false) {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET vSection6Selection " + "6" + ";");
				...
				for(Inning inn : match.getMatch().getInning()) {
					if (inn.getIsCurrentInning().toUpperCase().equalsIgnoreCase(CricketUtil.YES)) {
						...
						if(inn.getInningNumber() == 1 || inn.getInningNumber() == 2) {
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFreeTextSec4 " + "1st INNINGS" + ";");
						}else if(inn.getInningNumber() == 3 || inn.getInningNumber() == 4) {
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFreeTextSec4 " + "2nd INNINGS" + ";");
						}
					}
				}
				break;
			case "DAY_SESSION":
				...
				if(match.getMatch().getDaysSessions().get(match.getMatch().getDaysSessions().size()-1).getIsCurrentSession().equalsIgnoreCase(CricketUtil.YES)) {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFreeTextSec4 " + 
			...
			case "VS_BOWLING_TEAM":
				for(Inning inn : match.getMatch().getInning()) {
					if (inn.getIsCurrentInning().toUpperCase().equalsIgnoreCase(CricketUtil.YES)) {
						...
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBallTeamName " + 
				...
				for(Inning inn : match.getMatch().getInning()) {
					...
					if(inn.getIsCurrentInning().equalsIgnoreCase(CricketUtil.YES)) {
						...
						if(inn.getRunRate() != null) {
							...
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tCurrentRunRate " + "0.0" + ";");
						}
			...
			case"TARGET":
				...
				//print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tCurrentRunRateText " + "CRR@ " + ";");
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTargetScore " + CricketFunctions.GetTargetData(match).getTargetRuns() + ";");
				break;
	...
	public Infobar populateInfobarBottomRight(Infobar infobar,boolean is_this_updating, PrintWriter print_writer, MatchAllData match, String session_selected_broadcaster)
	{
		...
		case "ASSAM":
			...
			case "BALL_SINCE":
				for(Inning inn : match.getMatch().getInning()) {
					if(inn.getIsCurrentInning().equalsIgnoreCase(CricketUtil.YES)) {
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFreeTextSmallText " + "BALLS SINCE LAST BOUNDARY "  + CricketFunctions.lastFewOversData(CricketUtil.BOUNDARY, match.getEventFile().getEvents(),inn.getInningNumber()) + ";");
					}
				}
			...
			case "TOSS_WINNING":
				...
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTossResult " + CricketFunctions.generateTossResult(match, "", "", CricketUtil.SHORT,CricketUtil.CHOSE).toUpperCase() + ";");
				break;
			case "SUPER_OVER":
				if(is_this_updating == false) {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET vSection6Selection " + "6" + ";");
				...
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFreeTextSmallText " + "SUPER OVER" + ";");
				break;
			case "TOURNAMENT_NAME":
				...
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFreeTextSmallText " + match.getSetup().getTournament().toUpperCase() + ";");
				break;
			case "VENUE_NAME":
				if(is_this_updating == false) {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET vSection6Selection " + "6" + ";");
				...
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFreeTextSmallText " + match.getSetup().getVenueName().toUpperCase() + ";");
				break;
			case "EQUATION":
				if(is_this_updating == false) {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET vSection6Selection " + "6" + ";");
				...
				if(match.getSetup().getMatchType().equalsIgnoreCase(CricketUtil.TEST)) {
					if(match.getMatch().getInning().get(1).getIsCurrentInning().equalsIgnoreCase(CricketUtil.YES)) {
						...
						if(CricketFunctions.GetTeamRunsAhead(2,match) > 0) {
							...
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFreeTextSmallText " + "SCORES ARE LEVEL" + ";");
						} else {
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFreeTextSmallText " + 
						...
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFreeTextSmallText " + match.getMatch().getMatchStatus().toUpperCase() + ";");
					}
				}else {
						if(match.getSetup().getTargetOvers() == null || match.getSetup().getTargetOvers().trim().isEmpty() && match.getSetup().getTargetRuns() == 0) {
							if(CricketFunctions.GetTargetData(match).getRemaningRuns() == 0) {
								if(match.getMatch().getMatchStatus() != null) {
									if(match.getMatch().getMatchResult().toUpperCase().equalsIgnoreCase("DRAWN")){
										...
										print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFreeTextSmallText " + "MATCH TIED" + ";");
									}
									else if(match.getMatch().getMatchResult().split(",")[1].toUpperCase().equalsIgnoreCase("SUPER_OVER")){
										print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET vSection5Selection " + "1" + ";");
									...
									|| match.getMatch().getInning().get(1).getTotalOvers() >= match.getSetup().getMaxOvers()) 
							{
								...
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tRuns " + "RUN" + CricketFunctions.Plural(CricketFunctions.GetTargetData(match).getRemaningRuns()).toUpperCase() + ";");
								if (CricketFunctions.GetTargetData(match).getRemaningBall() >= 100) {
									print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tNeedBall " + CricketFunctions.OverBalls(0,CricketFunctions.GetTargetData(match).getRemaningBall()) + ";");
									print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBalls " + "OVERS" + ";");
								}else {
									...
									print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBalls " + "BALL" + CricketFunctions.Plural(CricketFunctions.GetTargetData(match).getRemaningBall()).toUpperCase() + ";");
								}
							...
							if(Double.valueOf(match.getSetup().getTargetOvers()) == 1 && match.getSetup().getTargetRuns() == 0) {
										...
										>= Double.valueOf(match.getSetup().getTargetOvers())) 
								{
									...
									if(match.getSetup().getTargetType() != null) {
										...
										if (CricketFunctions.GetTargetData(match).getRemaningBall() >= 100) {
											print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tNeedBall " + CricketFunctions.OverBalls(0,CricketFunctions.GetTargetData(match).getRemaningBall()) + ";");
											print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBalls " + "OVERS" + " (" + match.getSetup().getTargetType().toUpperCase() + ")" + ";");
										}else {
											print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tNeedBall " + CricketFunctions.GetTargetData(match).getRemaningBall() + ";");
											print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBalls " + "BALL" + CricketFunctions.Plural(CricketFunctions.GetTargetData(match).getRemaningBall()).toUpperCase() + " (" + match.getSetup().getTargetType().toUpperCase() + ")" + ";");
										}
									}
									else {
										print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tNeedRun " + CricketFunctions.GetTargetData(match).getRemaningRuns() + ";");
										...
										if (CricketFunctions.GetTargetData(match).getRemaningBall() >= 100) {
											...
											print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tNeedBall " + CricketFunctions.GetTargetData(match).getRemaningBall() + ";");
											print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBalls " + "BALL" + CricketFunctions.Plural(CricketFunctions.GetTargetData(match).getRemaningBall()) + ";");
										}
									}
								}
							}
			...
			case "REMAINING_OVERS":
				...
				if(remain_overs % 6 == 0) {
					...
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFreeTextSmallText " + "REMAINING OVERS : " + remain_overs + ";");
					
				}else {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFreeTextSmallText " + "REMAINING OVERS : " + CricketFunctions.OverBalls(0, remain_overs) + ";");
				}
			...
			case "FOLLOW_ON":
				...
				if(rem_runs > 0) {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFreeTextSmallText " + "RUNS TO AVOID FOLLOW-ON : " + rem_runs + ";");
				}else {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFreeTextSmallText " + "FOLLOW-ON AVOIDED" + ";");
				}
				break;
			...
			case"COMPARISION":
				...
				for(Inning inn : match.getMatch().getInning()) {
					if(inn.getInningNumber() == 1 & inn.getIsCurrentInning().toUpperCase().equalsIgnoreCase("NO")) {
						...
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBallTeamName " + match.getMatch().getInning().get(0).getBatting_team().getTeamName1().toUpperCase() + ";");
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBallTeamScore " + CricketFunctions.compareInningData(match,"/", 1 , match.getEventFile().getEvents()) + ";");
					}
				}
				break;
			}
	...
	public Infobar populateInfobarBottom(Infobar infobar,boolean is_this_updating, PrintWriter print_writer, MatchAllData match, String session_selected_broadcaster)
	{
		...
		case "ASSAM":
			...
			case "EXTRAS":
				if(is_this_updating == false) {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET vSection4Selection " + "5" + ";");
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET vSection5Selection " + "4" + ";");
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET vSection6Selection " + "3" + ";");
				}
				for(Inning inn : match.getMatch().getInning()) {
					if(inn.getIsCurrentInning().equalsIgnoreCase(CricketUtil.YES)) {
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFreeTextSec6 " + "NB " + inn.getTotalNoBalls() + ", WD " 
							  ...
							  {  
								...
								case CricketUtil.PENALTY: case CricketUtil.LOG_WICKET: case CricketUtil.LOG_ANY_BALL:
								    ...
								    case CricketUtil.CHANGE_BOWLER:
										print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET vBall" + ball_count + " " + "0" + ";");
										break;
								    case CricketUtil.ONE : case CricketUtil.TWO: case CricketUtil.THREE:  case CricketUtil.FIVE : case CricketUtil.DOT:
								    	print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET vBall" + ball_count + " " + "2" + ";");
										print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTimelineRun"+ ball_count + " " + match.getEventFile().getEvents().get(i).getEventRuns() + ";");
										break;
								    case CricketUtil.FOUR: case CricketUtil.SIX: 
								    	print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET vBall" + ball_count + " " + "3" + ";");
										print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTimelineRun"+ ball_count + " " + match.getEventFile().getEvents().get(i).getEventRuns() + ";");
										break;
								    case CricketUtil.WIDE: case CricketUtil.NO_BALL: case CricketUtil.BYE: case CricketUtil.LEG_BYE: case CricketUtil.PENALTY: 
								    	print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET vBall" + ball_count + " " + "5" + ";");
										...
										print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTimelineRun"+ ball_count + " " + 
								    ...
								    case CricketUtil.LOG_WICKET: 
								    	if (match.getEventFile().getEvents().get(i).getEventRuns() > 0) {
								    	  print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET vBall" + ball_count + " " + "15" + ";");
								    	  print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTimelineRun"+ ball_count + " " + 
								    			  String.valueOf(match.getEventFile().getEvents().get(i).getEventRuns()) + "+W" + ";");
								      } else {
								    	  print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET vBall" + ball_count + " " + "15" + ";");
								    	  print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTimelineRun"+ ball_count + " " + "W" + ";");
								      }
								      break;
	...
	public void populateMatchPromo(PrintWriter print_writer,String viz_sence_path, int match_number ,List<Team> team,List<Fixture> fix,MatchAllData match, String session_selected_broadcaster) throws InterruptedException 
	{
		...
		case "ASSAM":
			if (match == null) {
				...
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tHeader " + "MATCH " + fix.get(match_number - 1).getMatchnumber() + ";");
				...
				for(Team TM : team) {
					if(fix.get(match_number - 1).getHometeamid() == TM.getTeamId()) {
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgHomeTeamLogo " + logo_path + TM.getTeamName1().toUpperCase() + CricketUtil.PNG_EXTENSION + ";");
					}
					if(fix.get(match_number - 1).getAwayteamid() == TM.getTeamId()) {
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgAwayTeamLogo " + logo_path + TM.getTeamName1().toUpperCase() + CricketUtil.PNG_EXTENSION + ";");
					}
				}
				...
				if(fix.get(match_number - 1).getDate().equalsIgnoreCase(new SimpleDateFormat("dd-MM-yyyy").format(cal.getTime()))) {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFooter " + "TOMORROW " + "- LIVE FROM "+ match.getSetup().getVenueName().toUpperCase() +";");
				}else {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFooter " + "UP NEXT " + "- LIVE FROM "+ match.getSetup().getVenueName().toUpperCase()  + ";");
	...
	public void populateMatchId(PrintWriter print_writer,String viz_scene,List<VariousText> vt, MatchAllData match, String session_selected_broadcaster) throws InterruptedException
	{
		...
		case "ASSAM":
			if (match == null) {
				...
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tHeader " + match.getSetup().getMatchIdent().toUpperCase()+ ";");
				//print_writer.println("LAYER1*EVEREST*TREEVIEW*Subheader1*CONTAINER SET ACTIVE 0;");
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSubHeader " + match.getSetup().getTournament().toUpperCase() + ";");
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgTeamLogo " + logo_path + "TLogo" + CricketUtil.PNG_EXTENSION + ";");
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgHomeTeamLogo " + logo_path + match.getSetup().getHomeTeam().getTeamName1().toUpperCase() + CricketUtil.PNG_EXTENSION + ";");
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgAwayTeamLogo " + logo_path + match.getSetup().getAwayTeam().getTeamName1().toUpperCase() + CricketUtil.PNG_EXTENSION + ";");
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFooter " + "LIVE FROM "+ match.getSetup().getVenueName().toUpperCase() + ";");
	...
	public void populateLTMatchId(PrintWriter print_writer,String viz_scene, MatchAllData match, String session_selected_broadcaster) throws InterruptedException
	{
		...
		case "DOAD_IN_HOUSE_VIZ":
			if (match == null) {
				...
				print_writer.println("-1 RENDERER*TREE*$Main$All$DataAll$All_Name$HomeTeamName$HomeTeamFirstName*GEOM*TEXT SET "+ match.getSetup().getHomeTeam().getTeamName1().toUpperCase() + " \0");
				...
				print_writer.println("-1 RENDERER*TREE*$Main$All$DataAll$All_Name$AwayTeamName$AwayTeamFirstName*GEOM*TEXT SET "+ match.getSetup().getAwayTeam().getTeamName1().toUpperCase() + " \0");
				
				print_writer.println("-1 RENDERER*TREE*$Main$All$DataAll$All_Stats$StatAll1$Info*GEOM*TEXT SET "+ match.getSetup().getTournament().toUpperCase() + " LIVE FROM " 
				...
				if(TeamId == match.getSetup().getHomeTeamId()) {
					...
					for(Player hs : match.getSetup().getHomeSquad()) {
						...
						if(config.getPrimaryIpAddress().equalsIgnoreCase("LOCALHOST")) {
							...
							if(!new File("\\\\"+config.getPrimaryIpAddress()+"\\c\\Images\\ASSAM\\Photos\\" +match.getSetup().getHomeTeam().getTeamName4().toUpperCase()
									+ "\\\\" + hs.getPhoto() + CricketUtil.PNG_EXTENSION).exists()) {
								this.status = CricketUtil.UNSUCCESSFUL;
							}
							print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgPlayerImage0"+ row_id + " " + "\\\\\\\\"+config.getPrimaryIpAddress()+"\\\\"+local_photo_path + 
									match.getSetup().getHomeTeam().getTeamName4().toUpperCase() + "\\\\" + hs.getPhoto() + CricketUtil.PNG_EXTENSION + ";");
						}
					...
					for(Player as : match.getSetup().getAwaySquad()) {
						...
						if(config.getPrimaryIpAddress().equalsIgnoreCase("LOCALHOST")) {
							...
							if(!new File("\\\\"+config.getPrimaryIpAddress()+"\\c\\Images\\ASSAM\\Photos\\" +match.getSetup().getAwayTeam().getTeamName4().toUpperCase()
			...
			if (match == null) {
				...
				for(Inning inn : match.getMatch().getInning()) {
					if(inn.getInningNumber() == 1 & inn.getIsCurrentInning().equalsIgnoreCase("YES")) {
						...
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStateHead1 " + "@"+ proj_score_rate[0] +" (CRR)" + ";");
		...
		case "ASSAM":
			if (match == null) {
				...
				//if(inn.getInningNumber() == 2 && inn.getIsCurrentInning().equalsIgnoreCase("YES")) {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFirstName " +  "TARGET" +";");
					...
					if(match.getSetup().getTargetType() == null || match.getSetup().getTargetType().trim().isEmpty()) {
						if(CricketFunctions.GetTargetData(match).getTargetOvers() == "1") {
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tDetails " + match.getMatch().getInning().get(1).getBatting_team().getTeamName3().toUpperCase() + " NEED " + 
...
//				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTeamLastName " + match.getMatchIdent().toUpperCase() + ";");
				...
				for(Inning inn : match.getMatch().getInning()) {
					//if(inn.getInningNumber() == 2 && inn.getIsCurrentInning().equalsIgnoreCase("YES")) {
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTeamFirstName " + inn.getBatting_team().getTeamName1().toUpperCase() + ";");
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSubHeader01 " + "v " + inn.getBowling_team().getTeamName1().toUpperCase() + ";");
						
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgAwayTeamLogo " + logo_path + inn.getBatting_team().getTeamName4() + CricketUtil.PNG_EXTENSION + ";");
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgHomeTeamLogo " + logo_path + inn.getBowling_team().getTeamName4() + CricketUtil.PNG_EXTENSION + ";");
						
						
						if(match.getSetup().getTargetType() == null || match.getSetup().getTargetType().trim().isEmpty()) {
							if(CricketFunctions.GetTargetData(match).getTargetOvers() == "1") {
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTargetRuns " + CricketFunctions.GetTargetData(match).getTargetRuns() + ";");
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tRunsHead " + "RUNS" + ";");
								
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTargetBalls " + (Double.valueOf(CricketFunctions.GetTargetData(match).getTargetOvers())*6) + ";");
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBallsHead " + "BALLS" + ";");
							}else {
								if(Double.valueOf(CricketFunctions.GetTargetData(match).getTargetOvers())*6 >= 100) {
									print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTargetRuns " + CricketFunctions.GetTargetData(match).getTargetRuns() + ";");
									print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tRunsHead " + "RUNS" + ";");
									
									print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTargetBalls " + match.getSetup().getMaxOvers() + ";");
									print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBallsHead " + "OVERS" + ";");
								}else {
									print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTargetRuns " + CricketFunctions.GetTargetData(match).getTargetRuns() + ";");
									print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tRunsHead " + "RUNS" + ";");
									
									print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTargetBalls " + (Integer.valueOf(CricketFunctions.GetTargetData(match).getTargetOvers())*6) + ";");
									print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBallsHead " + "BALLS" + ";");
								}
							}
						}else {
							if(Double.valueOf(CricketFunctions.GetTargetData(match).getTargetOvers())*6 >= 100) {
								...
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTargetBalls " + Double.valueOf(CricketFunctions.GetTargetData(match).getTargetOvers()) + ";");
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBallsHead " + "OVERS" + " (" + match.getSetup().getTargetType().toUpperCase() + ")" + ";");
							}else {
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTargetRuns " + CricketFunctions.GetTargetData(match).getTargetRuns() + ";");
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tRunsHead " + "RUNS" + ";");
								
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTargetBalls " + Double.valueOf(CricketFunctions.GetTargetData(match).getTargetOvers())*6 + ";");
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tBallsHead " + "BALLS" + " ("+ match.getSetup().getTargetType().toUpperCase() +")" + ";");
							}
						}
						
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tCRRHead " + " " + ";");
						requiredRuns = match.getMatch().getInning().get(0).getTotalRuns();
						...
						System.out.println(requiredBalls + "  " + requiredRuns);
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tCRR " + "REQUIRED RUN RATE : " + CricketFunctions.generateRunRate
								(requiredRuns, 0, requiredBalls, 2,match) + ";");
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tRRRHead " + " " + ";");
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tRRR " + " " + ";");
					//}
				}
			}
		...
		case "ASSAM":
			if (match == null) {
				...
				for(Inning inn : match.getMatch().getInning()) {
					if (inn.getInningNumber() == whichInning) {
						if (inn.getBowlingTeamId() == match.getSetup().getHomeTeamId()) {
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTeamName " + match.getSetup().getAwayTeam().getTeamName4().toUpperCase() + ";");
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgTeamLogo " + logo_path + match.getSetup().getAwayTeam().getTeamName4() + CricketUtil.PNG_EXTENSION + ";");
						} else {
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTeamName " + match.getSetup().getHomeTeam().getTeamName4().toUpperCase() + ";");
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgTeamLogo " + logo_path  + match.getSetup().getHomeTeam().getTeamName4() + CricketUtil.PNG_EXTENSION + ";");
						}
						if(inn.getTotalWickets() >= 10) {
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tScore " + inn.getTotalRuns() + ";");
	...
	public void populateSplit(PrintWriter print_writer,String viz_scene,int whichInning,int splitValue, MatchAllData match, String session_selected_broadcaster)
	{
		...
		case "ASSAM":
			if (match == null) {
				...
				for(Inning inn : match.getMatch().getInning()) {
					if (inn.getInningNumber() == whichInning) {
						if (inn.getBowlingTeamId() == match.getSetup().getHomeTeamId() && splitValue == 30 || splitValue == 50) {
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTeamName " + match.getSetup().getAwayTeam().getTeamName4().toUpperCase() + ";");
			...
			if (match == null) {
				...
				for(Inning inn : match.getMatch().getInning()) {
					if(inn.getInningNumber() == 2 & inn.getIsCurrentInning().equalsIgnoreCase("YES")) {
						...
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSubHead1 " + inn.getBowling_team().getTeamName4().toUpperCase() + " WERE " + CricketFunctions.compareInningData(match, "-", 1, match.getEventFile().getEvents()) + ";");
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSubHead2 " + inn.getBatting_team().getTeamName4().toUpperCase() + " ARE " + CricketFunctions.compareInningData(match, "-", 2, match.getEventFile().getEvents()) + ";");
						...
						for(Player hs : match.getSetup().getHomeSquad()) {
							if(hs.getPlayerId() == inn.getPartnerships().get(inn.getPartnerships().size() - 1).getFirstBatterNo()) {
								...
								if(config.getPrimaryIpAddress().equalsIgnoreCase("LOCALHOST")) {
									print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgPlayerImage01 " + photo_path + match.getSetup().getHomeTeam().getTeamName4().toUpperCase() + 
											"\\\\" + hs.getPhoto() + CricketUtil.PNG_EXTENSION + ";");
								}else {
									if(!new File("\\\\"+config.getPrimaryIpAddress()+"\\c\\Images\\ASSAM\\Photos\\" +match.getSetup().getHomeTeam().getTeamName4().toUpperCase()
											+ "\\\\" + hs.getPhoto() + CricketUtil.PNG_EXTENSION).exists()) {
										this.status = CricketUtil.UNSUCCESSFUL;
									}
									print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgPlayerImage01 " + "\\\\\\\\"+config.getPrimaryIpAddress()+"\\\\"+local_photo_path + match.getSetup().getHomeTeam().getTeamName4().toUpperCase() + 
											"\\\\" + hs.getPhoto() + CricketUtil.PNG_EXTENSION + ";");
								}	
							}
							if(hs.getPlayerId() == inn.getPartnerships().get(inn.getPartnerships().size() - 1).getSecondBatterNo()) {
								...
								if(config.getPrimaryIpAddress().equalsIgnoreCase("LOCALHOST")) {
									...
									print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgPlayerImage02 " + "\\\\\\\\"+config.getPrimaryIpAddress()+"\\\\"+local_photo_path + match.getSetup().getHomeTeam().getTeamName4().toUpperCase() + 
											"\\\\" + hs.getPhoto() + CricketUtil.PNG_EXTENSION + ";");
								}	
							}
						}
						...
						for(Player as : match.getSetup().getAwaySquad()) {
							if(as.getPlayerId() == inn.getPartnerships().get(inn.getPartnerships().size() - 1).getFirstBatterNo()) {
								...
								if(config.getPrimaryIpAddress().equalsIgnoreCase("LOCALHOST")) {
									print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgPlayerImage01 " + photo_path + match.getSetup().getAwayTeam().getTeamName4().toUpperCase() + 
											"\\\\" + as.getPhoto() + CricketUtil.PNG_EXTENSION + ";");
								}else {
									if(!new File("\\\\"+config.getPrimaryIpAddress()+"\\c\\Images\\ASSAM\\Photos\\" +match.getSetup().getAwayTeam().getTeamName4().toUpperCase()
											+ "\\\\" + as.getPhoto() + CricketUtil.PNG_EXTENSION).exists()) {
										this.status = CricketUtil.UNSUCCESSFUL;
									}
									print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgPlayerImage01 " + "\\\\\\\\"+config.getPrimaryIpAddress()+"\\\\"+local_photo_path + match.getSetup().getAwayTeam().getTeamName4().toUpperCase() + 
											"\\\\" + as.getPhoto() + CricketUtil.PNG_EXTENSION + ";");
								}	
							}
							if(as.getPlayerId() == inn.getPartnerships().get(inn.getPartnerships().size() - 1).getSecondBatterNo()) {
								...
								if(config.getPrimaryIpAddress().equalsIgnoreCase("LOCALHOST")) {
									...
									print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgPlayerImage02 " + "\\\\\\\\"+config.getPrimaryIpAddress()+"\\\\"+local_photo_path + match.getSetup().getAwayTeam().getTeamName4().toUpperCase() + 
											"\\\\" + as.getPhoto() + CricketUtil.PNG_EXTENSION + ";");
								}	
							}
						}
	...
	public void populateLeaderBoard(PrintWriter print_writer,String viz_scene, String FirstPlayer, String SecondPlayer , MatchAllData match, String session_selected_broadcaster) 
	{
		if (match == null) {
			this.status = "ERROR: Match is null";
		} else if (match.getMatch().getInning() == null) {
	...
	public void populateLtEquation(PrintWriter print_writer,String viz_scene, MatchAllData match, String session_selected_broadcaster) throws InterruptedException
	{
		...
		case "ASSAM":
			if (match == null) {
				...
				for(Inning inn : match.getMatch().getInning()) {
					if(inn.getInningNumber() == 2 && inn.getIsCurrentInning().equalsIgnoreCase("YES")) {
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFirstName " +  "EQUATION" +";");
						
						if(match.getSetup().getTargetOvers() == null || match.getSetup().getTargetOvers().trim().isEmpty() && match.getSetup().getTargetRuns() == 0) {
							if(CricketFunctions.GetTargetData(match).getRemaningRuns() == 0) {
								if(match.getMatch().getMatchStatus() != null) {
									if(match.getMatch().getMatchResult().toUpperCase().equalsIgnoreCase("DRAWN")){
										print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tDetails " + "MATCH TIED" + ";");
									}
									else if(match.getMatch().getMatchResult().split(",")[1].toUpperCase().equalsIgnoreCase("SUPER_OVER")){
										print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tDetails " + "MATCH TIED - " + match.getMatch().getMatchStatus().toUpperCase() + ";");
									}
									else {
										print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tDetails " + match.getMatch().getMatchStatus().toUpperCase() + ";");
									}
								}
								...
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tDetails " + inn.getBatting_team().getTeamName4().toUpperCase() 
										...
										" FROM "+ CricketFunctions.GetTargetData(match).getRemaningBall() + " BALL" + CricketFunctions.Plural(CricketFunctions.GetTargetData(match).getRemaningBall()).toUpperCase() +  ";");
							...
							if(Double.valueOf(match.getSetup().getTargetOvers()) == 1 && match.getSetup().getTargetRuns() == 0) {
								...
								if((match.getSetup().getTargetRuns() - match.getMatch().getInning().get(1).getTotalRuns()) == 0) {
									...
									if(match.getSetup().getTargetType().toUpperCase().equalsIgnoreCase("VJD")) {
										print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tDetails " + inn.getBatting_team().getTeamName4().toUpperCase() 
												+ " NEED " + CricketFunctions.GetTargetData(match).getRemaningRuns() + " RUN" + CricketFunctions.Plural(CricketFunctions.GetTargetData(match).getRemaningRuns()).toUpperCase() + 
												" FROM "+ CricketFunctions.GetTargetData(match).getRemaningBall() + " BALL" + CricketFunctions.Plural(CricketFunctions.GetTargetData(match).getRemaningBall()).toUpperCase() + " (VJD)" +  ";");
									}
									else {
										print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tDetails " + inn.getBatting_team().getTeamName4().toUpperCase() 
												+ " NEED " + CricketFunctions.GetTargetData(match).getRemaningRuns() + " RUN" + CricketFunctions.Plural(CricketFunctions.GetTargetData(match).getRemaningRuns()).toUpperCase() + 
	...
	public void populatePointsTable(PrintWriter print_writer,String viz_sence_path,List<LeagueTeam> point_table, String session_selected_broadcaster,MatchAllData match) throws InterruptedException 
	{
		...
		case "ASSAM":
			int row_no=0;
			print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSubHeader01 " + match.getSetup().getTournament().toUpperCase() + ";");
			print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgTeamLogo " + logo_path + "TLogo" + CricketUtil.PNG_EXTENSION + ";");
			for(int i = 0; i <= point_table.size() - 1 ; i++) {
				...
				if(point_table.get(i).getQualifiedStatus().trim().equalsIgnoreCase("")) {
					...
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTeamName0" + row_no + " " + point_table.get(i).getTeamName().toUpperCase() + ";");
				}else {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTeamName0" + row_no + " " + "(Q) " + point_table.get(i).getTeamName().toUpperCase() + ";");
				}
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatValue" + (row_no + 1) + "A" + " " + point_table.get(i).getPlayed() + ";");
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatValue" + (row_no + 1) + "B" + " " + point_table.get(i).getWon() + ";");
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatValue" + (row_no + 1) + "C" + " " + point_table.get(i).getLost() + ";");
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatValue" + (row_no + 1) + "D" + " " + point_table.get(i).getNoResult() + ";");
				DecimalFormat df = new DecimalFormat("0.000");
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatHeadE " + "POINTS" + ";");
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatHeadF " + "NRR" + ";");
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatValue" + (row_no + 1) + "F" + " " + df.format(point_table.get(i).getNetRunRate()) + ";");
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatValue" + (row_no + 1) + "E" + " " + point_table.get(i).getPoints() + ";");
			...
			if (match == null) {
				...
				for(Player plr : plyr) {
					if(plr.getPlayerId() ==playerId) {
						...
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgTeamLogo " + logo_path + team.get(plr.getTeamId() - 1).
								getTeamName4() + CricketUtil.PNG_EXTENSION + ";");
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tFirstName " + plr.getFull_name().toUpperCase() +";");
						if(plr.getBowlingStyle() != null) {
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tDetails " + CricketFunctions.getbowlingstyle(plr.getBowlingStyle()).toUpperCase() + ";");
						}else {
	...
	public void populateTieIdDouble(PrintWriter print_writer,String viz_sence_path,String day,List<Fixture> fix,List<Team> team,MatchAllData match, String selectedbroadcaster) throws InterruptedException 
	{
		if (match == null) {
			...
			if(day.toUpperCase().equalsIgnoreCase("TODAY")) {
				...
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tHeader " + "TODAY'S MATCHES " +";");
			}
			else if(day.toUpperCase().equalsIgnoreCase("TOMORROW")) {
				cal.add(Calendar.DATE, +1);
				Date =  new SimpleDateFormat("dd-MM-yyyy").format(cal.getTime());
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tHeader " + "TOMORROW'S MATCHES " +";");
			}
			...
			for(int i = 0; i <= fix.size()-1; i++) {
				if(fix.get(i).getDate().equalsIgnoreCase(Date)) {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgHomeTeam" +row_id+ "Logo " + logo_path + team.get(fix.get(i).getHometeamid()-1).getTeamName4().toUpperCase() 
							 + CricketUtil.PNG_EXTENSION +";");
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tMatch" + row_id + " " + "MATCH " + fix.get(i).getMatchnumber() +";");
					
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgAwayTeam" +row_id+ "Logo " + logo_path + team.get(fix.get(i).getAwayteamid()-1).getTeamName4().toUpperCase() 
							 + CricketUtil.PNG_EXTENSION +";");
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tMatch" + row_id + " " + "MATCH " +fix.get(i).getMatchnumber() +";");
				 row_id = row_id +1;
				}
		...
		print_writer.println("LAYER1*EVEREST*STAGE*DIRECTOR*In STOP;");
		print_writer.println("LAYER1*EVEREST*STAGE*DIRECTOR*Out STOP;");
		print_writer.println("LAYER1*EVEREST*STAGE*DIRECTOR*In SHOW 70.0;");
		print_writer.println("LAYER1*EVEREST*STAGE*DIRECTOR*Out SHOW 0.0;");
		print_writer.println("LAYER1*EVEREST*GLOBAL SNAPSHOT_PATH C:/Temp/Preview.bmp;");
		print_writer.println("LAYER1*EVEREST*GLOBAL SNAPSHOT 1920 1080;");
		TimeUnit.SECONDS.sleep(1);
		...
		case "ASSAM":
			if (match == null) {
				...
				for(int j = 0; j <= mtch.size() - 1; j++) {
					...
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTeamFirstName " + mtch.get(j).getSetup().getMatchIdent().toUpperCase() +  ";");
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTeamLastName " + " SUMMARY" + ";");
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSubHeader01 " + mtch.get(j).getSetup().getHomeTeam().getTeamName1().toUpperCase() + 
					...
					for(int i = 1; i <= 2 ; i++) {
						...
						if(mtch.get(j).getMatch().getInning().get(i-1).getTotalWickets() >= 10) {
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tScore0"+ row + " " + mtch.get(j).getMatch().getInning().get(i-1).getTotalRuns() + ";");
						}
						else {
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tScore0"+ row + " " + mtch.get(j).getMatch().getInning().get(i-1).getTotalRuns() 
									+ slashOrDash + mtch.get(j).getMatch().getInning().get(i-1).getTotalWickets() + ";");	
						}
						...
						for(int k = row_id + 1; k <= max_Strap; k++) {
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Batsman0"+ k + "*CONTAINER SET ACTIVE 0;");
						}
						...
						for(int k = row_id + 1; k <= max_Strap; k++) {
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Bowler0"+ k + "*CONTAINER SET ACTIVE 0;");
						}
					}
					...
					if(mtch.get(j).getMatch().getMatchResult() != null) {
						if(mtch.get(j).getMatch().getMatchResult().toUpperCase().equalsIgnoreCase("DRAWN")){
							...
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tResult " + "MATCH TIED - " + mtch.get(j).getMatch().getMatchStatus().toUpperCase() + ";");
						}
						else {
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tResult " + mtch.get(j).getMatch().getMatchStatus().toUpperCase() + ";");
						}
					}
					else {
				...
				for(Player plr : plyr) {
					if(plr.getPlayerId() ==playerId) {
						...
						if(plr.getBattingStyle() != null) {
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tDetails " + CricketFunctions.getbattingstyle(plr.getBattingStyle(), CricketUtil.FULL, false, false).toUpperCase() + ";");
						}else {
							print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tDetails " + " " + ";");
						}
					}
	...
	public void populateManhattan(PrintWriter print_writer,String viz_scene, int whichInning, MatchAllData match, String session_selected_broadcaster) throws InterruptedException
	{
		...
		case "ASSAM":
			if (match == null) {
				...
				for(Inning inn : match.getMatch().getInning()) {
					if (inn.getInningNumber() == whichInning) {
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgTeamLogo " + logo_path + inn.getBatting_team().getTeamName1() + 
								CricketUtil.PNG_EXTENSION + ";");
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTeamFirstName " + inn.getBatting_team().getTeamName1().toUpperCase() + ";");
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSubHeader01 " + "v " + inn.getBowling_team().getTeamName1().toUpperCase() + ";");
					}
				...
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main$DataAll$BAtting$Manhatten$Main$Graph$Bars*GEOMETRY*BARS SET MAX_HEIGHT_VALUE " + maxRuns + ";");
				print_writer.println("LAYER1*EVEREST*TREEVIEW*Main$DataAll$BAtting$Manhatten$Main$Graph$Bars*GEOMETRY*BARS SET DATA " + cumm_runs.replace("0#", "") + ";");
				for(int j=1; j <= match.getSetup().getMaxOvers(); j++) {
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main$DataAll$BAtting$Manhatten$Main$Wickets$Over_" + j +"*CONTAINER SET ACTIVE 0;");
					...
					if(j < CricketFunctions.getOverByOverData(match,whichInning,"MANHATTAN", match.getEventFile().getEvents()).size()) {
						...
						System.out.println("runs = " + CricketFunctions.getOverByOverData(match,whichInning,"MANHATTAN", match.getEventFile().getEvents()).get(j).getOverTotalRuns());
//						if(j < 20) {
							if(CricketFunctions.getOverByOverData(match,whichInning,"MANHATTAN", match.getEventFile().getEvents()).get(j).getOverTotalWickets() > 0) {
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Over_" + (j) + "*FUNCTION*TAG_POSITION SET MAX_VALUE " + maxRuns + ";");
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET Over_" + (j) + " " + 
										CricketFunctions.getOverByOverData(match, whichInning,"MANHATTAN", match.getEventFile().getEvents()).get(j).getOverTotalRuns() + ";");
								
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main$DataAll$BAtting$Manhatten$Main$Wickets$Over_" + (j) +"*CONTAINER SET ACTIVE 1;");
								print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET Over"+ (j) + " " + (Integer.valueOf(
...
//						}
	...
	public void populateMostRuns(PrintWriter print_writer,String viz_scene,List<Tournament> tournament,MatchAllData match ,String session_selected_broadcaster) throws InterruptedException {
		...
		case "ASSAM":
			...
			print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSubHeader01 " + match.getSetup().getTournament().toUpperCase() + ";");
			print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgTeamLogo " + logo_path + "TLogo" + CricketUtil.PNG_EXTENSION + ";");
			print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTeamFirstName " + "MOST RUNS" + ";");
			...
			for(int i = 0; i <= tournament.size() - 1 ; i++) {
				...
				if(tournament.get(i).getRuns() > 0) {
					if(row_no < 11) {
						...
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatValue0" + row_no + "A" + " " + tournament.get(i).getPlayer().getFull_name().toUpperCase() + ";");
	...
	public void populateMostWickets(PrintWriter print_writer,String viz_scene,List<Tournament> tournament_boc,MatchAllData match ,String session_selected_broadcaster) throws InterruptedException {
		...
		case "ASSAM":
			...
			print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSubHeader01 " + match.getSetup().getTournament().toUpperCase() + ";");
			print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgTeamLogo " + logo_path + "TLogo" + CricketUtil.PNG_EXTENSION + ";");
			print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTeamFirstName " + "MOST WICKETS" + ";");
			...
			for(int i = 0; i <= tournament_boc.size() - 1 ; i++) {
				...
				if(tournament_boc.get(i).getWickets() > 0) {
					if(row_no < 10) {
						row_no = row_no + 1;
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET vCoumms " + row_no + ";");
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatValue0" + row_no + "A" + " " + tournament_boc.get(i).getPlayer().getFull_name().toUpperCase() + ";");
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatValue0" + row_no + "B" + " " + tournament_boc.get(i).getMatches() + ";");
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatValue0" + row_no + "C" + " " + tournament_boc.get(i).getWickets() + ";");
	...
	public void populateMostFours(PrintWriter print_writer,String viz_scene,List<Tournament> tournament_fours,MatchAllData match ,String session_selected_broadcaster) throws InterruptedException {
		...
		case "ASSAM":
			...
			print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSubHeader01 " + match.getSetup().getTournament().toUpperCase() + ";");
			print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgTeamLogo " + logo_path + "TLogo" + CricketUtil.PNG_EXTENSION + ";");
			print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTeamFirstName " + "MOST FOURS" + ";");
			
			print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET vRows " + "3" + ";");
			print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatHead01 " + "PLAYER" + ";");
			print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatHead02 " + "MATCHES" + ";");
			print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatHead03 " + "FOURS" + ";");
			//print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatHead04 " + "ECON." + ";");
			...
			for(int i = 0; i <= tournament_fours.size() - 1 ; i++) {
				...
				if(tournament_fours.get(i).getFours() > 0) {
					if(row_no < 10) {
						row_no = row_no + 1;
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET vCoumms " + row_no + ";");
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatValue0" + row_no + "A" + " " + tournament_fours.get(i).getPlayer().getFull_name().toUpperCase() + ";");
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatValue0" + row_no + "B" + " " + tournament_fours.get(i).getMatches() + ";");
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatValue0" + row_no + "C" + " " + tournament_fours.get(i).getFours() + ";");
					}
				}
			}
	...
	public void populateMostSixes(PrintWriter print_writer,String viz_scene,List<Tournament> tournament_sixes,MatchAllData match ,String session_selected_broadcaster) throws InterruptedException {
		...
		case "ASSAM":
			...
			print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSubHeader01 " + match.getSetup().getTournament().toUpperCase() + ";");
			print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgTeamLogo " + logo_path + "TLogo" + CricketUtil.PNG_EXTENSION + ";");
			print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTeamFirstName " + "MOST SIXES" + ";");
			
			print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET vRows " + "3" + ";");
			print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatHead01 " + "PLAYER" + ";");
			print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatHead02 " + "MATCHES" + ";");
			print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatHead03 " + "SIXES" + ";");
			//print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatHead04 " + "ECON." + ";");
			...
			for(int i = 0; i <= tournament_sixes.size() - 1 ; i++) {
				...
				if(tournament_sixes.get(i).getSixes() > 0) {
					if(row_no < 10) {
						row_no = row_no + 1;
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET vCoumms " + row_no + ";");
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatValue0" + row_no + "A" + " " + tournament_sixes.get(i).getPlayer().getFull_name().toUpperCase() + ";");
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatValue0" + row_no + "B" + " " + tournament_sixes.get(i).getMatches() + ";");
						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatValue0" + row_no + "C" + " " + tournament_sixes.get(i).getSixes() + ";");
					}
				}	
			}
	...
	public void populateHighestScore(PrintWriter print_writer,String viz_scene,List<Tournament> tournament_high_score,MatchAllData match ,String session_selected_broadcaster) throws InterruptedException {
		...
		case "ASSAM":
			...
			print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSubHeader01 " + match.getSetup().getTournament().toUpperCase() + ";");
			print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgTeamLogo " + logo_path + "TLogo" + CricketUtil.PNG_EXTENSION + ";");
			print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tTeamFirstName " + "HIGHEST INDIVIDUAL SCORE" + ";");
			...
			for(int i = 0; i <= top_ten_beststats.size() - 1 ; i++) {
				if(row_no < 10) {
					...
					print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tStatValue0" + row_no + "A" + " " + top_ten_beststats.get(i).getPlayer().getFull_name().toUpperCase() + ";");
...
//		}
	...
	public static String getInteractiveFile(MatchAllData match,List<Event> events) {
