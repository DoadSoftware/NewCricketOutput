...
public class Bukhatir extends Scene{

	private String status;
	private String slashOrDash = "-";
	public Infobar infobar = new Infobar();
	public String session_selected_broadcaster = "BUKHATIR";
	public String which_graphics_onscreen = "";
	private String logo_path = "C:\\Images\\Bukhatir\\Logos\\";
	private String photo_path = "C:\\Images\\Bukhatir\\Photos\\";
	private String local_photo_path = "\\\\c\\\\Images\\\\Bukhatir\\\\Photos\\\\";
	int whichInning;
	...
	public Bukhatir(String scene_path, String which_Layer) {
...
//		CricketFunctions.getInteractive(match);
	...
	public Object ProcessGraphicOption(String whatToProcess, MatchAllData match, CricketService cricketService, List<MatchAllData> tournament_matches,
			PrintWriter print_writer, List<Scene> scenes, String valueToProcess, List<Statistics> statistics, Configuration session_configuration, List<HeadToHeadPlayer> headToHead,
			List<Tournament> past_tournament_stats ) throws Exception{
		
		System.out.println("whatToProcess = " + whatToProcess.toUpperCase());
		...
		case "THIS_SESSION_GRAPHICS-OPTIONS": case "FF_EQUATION_GRAPHICS-OPTIONS": case "L3PLAYERPROFILEBUKHATIR_GRAPHICS-OPTIONS": case "PLAYERPROFILEBUKHATIR_GRAPHICS-OPTIONS":	
			...
			case "BUKHATIR": case "DOAD_IN_HOUSE_VIZ":
		...
		case "LEADERBOARD_GRAPHICS-OPTIONS": case "WICKETS_GRAPHICS-OPTIONS": case "FOURS_GRAPHICS-OPTIONS": case "SIXES_GRAPHICS-OPTIONS":
			...
			List<Tournament> tourn_stats = CricketFunctions.extractTournamentData("CURRENT_MATCH_DATA", false,headToHead, cricketService, match, past_tournament_stats);
		...
		case "POPULATE-FF-POSITION_LANDMARK": case "POPULATE-L3-BATSMAN_THIS_MATCH": case "POPULATE-L3-BOWLER_THIS_MATCH": case "POPULATE-BUKH-POINTSTABLE": case "POPULATE-LT-PARTNERSHIP":
		...
		case "POPULATE-BUG_POWERPLAY": case "POPULATE-BUG_PARTNERSHIP": case "POPULATE-L3-THISSERIES": case "POPULATE-LASTXBALL": case "POPULATE-DLS":
			switch (session_selected_broadcaster.toUpperCase()) {
			case "BUKHATIR":
				switch(whatToProcess.toUpperCase()) {
...
//					populateDuckWorthLewis(print_writer, valueToProcess.split(",")[0],valueToProcess.split(",")[1],valueToProcess.split(",")[2],valueToProcess.split(",")[3], match, session_selected_broadcaster);
				...
				case "POPULATE-MOSTRUNS":
					populateMostRuns(print_writer, valueToProcess.split(",")[0],
							CricketFunctions.extractTournamentData("CURRENT_MATCH_DATA", false,headToHead, cricketService, match, past_tournament_stats),
							/*CricketFunctions.extractTournamentStats("CURRENT_MATCH_DATA",false, tournament_matches, cricketService,match,null),*/
							match,session_selected_broadcaster);
					break;
				...
				case "POPULATE-LT-PARTNERSHIP":
					populateCurrentPartnership(print_writer, valueToProcess.split(",")[0], match, session_selected_broadcaster, session_configuration);
					break;
				...
				case "POPULATE-FF-PLAYINGXI":
					populatePlayingXI(print_writer, valueToProcess.split(",")[0], Integer.valueOf(valueToProcess.split(",")[1]), 
							match, session_selected_broadcaster, session_configuration);
					break;
				case "POPULATE-FF-LEADERBOARD":
					populateLeaderBoard(print_writer, valueToProcess.split(",")[0], valueToProcess.split(",")[1], Integer.valueOf(valueToProcess.split(",")[2]),
							/*CricketFunctions.extractTournamentStats("CURRENT_MATCH_DATA",false, tournament_matches, cricketService,match,null)*/
							CricketFunctions.extractTournamentData("CURRENT_MATCH_DATA", false,headToHead, cricketService, match, past_tournament_stats),
							cricketService.getTeams(),match, session_selected_broadcaster, session_configuration);
					break;
				...
				case "POPULATE-BUKH-POINTSTABLE":
					...
					populatePointsTable(print_writer, valueToProcess.split(",")[0],group.getLeagueTeams(),session_selected_broadcaster,match);
					break;
...
//							CricketUtil.MATCH , cricket_matches,true));	
		...
		case "ANIMATE-IN-THISSERIES": case "ANIMATE-IN-DLS":
			...
			case "BUKHATIR":
				...
				case "ANIMATE-IN-BUG-TOSS":	case "ANIMATE-IN-HOWOUT_WITHOUT_FIELDER_QUICK": case "ANIMATE-IN-BUG_PARTNERSHIP": case "ANIMATE-IN-THISSERIES": case "ANIMATE-IN-DLS":
					if(infobar.isInfobar_on_screen() == true) {
...
//					}
				...
				case "ANIMATE-OUT":
					...
					case "BUG_POWERPLAY": case "BUG_PARTNERSHIP": case "LEADERBOARD": case "THISSERIES": case "DLS":
		...
		case "BUKHATIR":
			...
			if(plyr.getTeamId() == match.getSetup().getHomeTeamId()) {
				if(config.getPrimaryIpAddress().equalsIgnoreCase("LOCALHOST")) {
					...
					if(!new File("\\\\"+config.getPrimaryIpAddress()+"\\c\\Images\\Bukhatir\\Photos\\" +match.getSetup().getHomeTeam().getTeamName4()
				...
				if(config.getPrimaryIpAddress().equalsIgnoreCase("LOCALHOST")) {
					...
					if(!new File("\\\\"+config.getPrimaryIpAddress()+"\\c\\Images\\Bukhatir\\Photos\\" +match.getSetup().getAwayTeam().getTeamName4()
							+"\\\\"+ plyr.getPhoto() + CricketUtil.PNG_EXTENSION).exists()) {
						this.status = CricketUtil.UNSUCCESSFUL;
					}
				...
				if(TeamId == match.getSetup().getHomeTeamId()) {
					...
					for(Player hs : match.getSetup().getHomeSquad()) {
						...
						if(config.getPrimaryIpAddress().equalsIgnoreCase("LOCALHOST")) {
							...
							if(!new File("\\\\"+config.getPrimaryIpAddress()+"\\c\\Images\\Bukhatir\\Photos\\" +match.getSetup().getHomeTeam().getTeamName4().toUpperCase()
					...
					for(Player as : match.getSetup().getAwaySquad()) {
						...
						if(config.getPrimaryIpAddress().equalsIgnoreCase("LOCALHOST")) {
							...
							if(!new File("\\\\"+config.getPrimaryIpAddress()+"\\c\\Images\\Bukhatir\\Photos\\" +match.getSetup().getAwayTeam().getTeamName4().toUpperCase()
...
//							print_writer.println("-1 RENDERER*TREE*$Main*FUNCTION*ControlObject*in SET ON " + "vHighlight" + row_no + " SET " + 1 + "\0");
							if(config.getPrimaryIpAddress().equalsIgnoreCase("LOCALHOST")) {
								...
								if(!new File("\\\\"+config.getPrimaryIpAddress()+"\\c\\Images\\Bukhatir\\Photos\\" +team.get(tournament.get(i).getPlayer().getTeamId() -1 ).getTeamName4()
...
//			}
	...
	public void populatePreviousSummary(PrintWriter print_writer, String viz_scene, int match_number,MatchAllData cricket_matches,List<Fixture> fix, MatchAllData match, String session_selected_broadcaster) throws InterruptedException
	{
		switch (session_selected_broadcaster.toUpperCase()) {
		case "BUKHATIR":
			if (match == null) {
				...
				this.status = "ERROR: Match Summary's inning is null";
					...
					print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSumHeader " + cricket_matches.getSetup().getMatchIdent().toUpperCase() + 
							" - SUMMARY" +  ";");
					print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSumSubHeader " + cricket_matches.getSetup().getHomeTeam().getTeamName1().toUpperCase() + 
							" vs " + cricket_matches.getSetup().getAwayTeam().getTeamName1().toUpperCase() + ";");
					print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET lgTLogo " + logo_path + "TLogo" + CricketUtil.PNG_EXTENSION + ";");
					print_writer.println("LAYER2*EVEREST*TREEVIEW*Main$Summary_GRP$DataAll$HeaderGrp$BigBandGrp$Sponsor*CONTAINER SET ACTIVE 0;");
					
					print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSumTeamALastName " + cricket_matches.getSetup().getMatchIdent().toUpperCase() + ";");
					print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSumTeamBLastName " + cricket_matches.getSetup().getMatchIdent().toUpperCase() + ";");
...
//					}
					...
					if(cricket_matches.getSetup().getMatchType().equalsIgnoreCase(CricketUtil.DT20)||cricket_matches.getSetup().getMatchType().equalsIgnoreCase(CricketUtil.ODI)||
							cricket_matches.getSetup().getMatchType().equalsIgnoreCase(CricketUtil.IT20)||cricket_matches.getSetup().getMatchType().equalsIgnoreCase(CricketUtil.D10)) {
...
//					}
					...
					for(int i = 1; i <= 2 ; i++) {
						...
						row = row + 1;
						if(cricket_matches.getMatch().getInning().get(i-1).getBattingTeamId() == cricket_matches.getSetup().getHomeTeamId()) {
							teamname = cricket_matches.getSetup().getHomeTeam().getTeamName1().toUpperCase();	
						} else {
							teamname = cricket_matches.getSetup().getAwayTeam().getTeamName1().toUpperCase();
						}
						
						if(cricket_matches.getSetup().getTossWinningTeam() == cricket_matches.getMatch().getInning().get(0).getBattingTeamId()) {
							print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET vHomeToss 1;");
							print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET vAwayToss 0;");
...
//							print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tInningHead1 " + "" + ";");
						...
						print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSumTeamName0"+ row +" "+ teamname + ";");
						print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSumOverNo0"+ row + " " + 
								CricketFunctions.OverBalls(cricket_matches.getMatch().getInning().get(i-1).getTotalOvers(),cricket_matches.getMatch().getInning().get(i-1).getTotalBalls()) + ";");
						
						if(cricket_matches.getMatch().getInning().get(i-1).getTotalWickets() >= 10) {
							print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSumScore0"+ row + " " + cricket_matches.getMatch().getInning().get(i-1).getTotalRuns() + ";");
						}
						else {
							print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSumScore0"+ row + " " + cricket_matches.getMatch().getInning().get(i-1).getTotalRuns() + slashOrDash +
										cricket_matches.getMatch().getInning().get(i-1).getTotalWickets() + ";");	
						}
						if(cricket_matches.getMatch().getInning().get(i-1).getBattingCard() != null) {
							//row_id = 0;
							Collections.sort(cricket_matches.getMatch().getInning().get(i-1).getBattingCard(),new CricketFunctions.BatsmenScoreComparator());
							for(BattingCard bc : cricket_matches.getMatch().getInning().get(i-1).getBattingCard()) {
								if(bc.getBatsmanInningStarted() != null && bc.getBatsmanInningStarted().equalsIgnoreCase(CricketUtil.YES) && bc.getStatus().equalsIgnoreCase(CricketUtil.STILL_TO_BAT)) {
									row_id = row_id + 1;
									print_writer.println("LAYER2*EVEREST*TREEVIEW*Main$Summary_GRP$DataAll$BAtting$Summary_Card$Batsman0"+ row_id + "*CONTAINER SET ACTIVE 1;");
									print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSumBatsmanName0"+ row_id + " " + bc.getPlayer().getTicker_name() + " (rh)" + ";");
						...
						if(cricket_matches.getMatch().getInning().get(i-1).getBowlingCard() != null) {
							//row_id = 0;
							Collections.sort(cricket_matches.getMatch().getInning().get(i-1).getBowlingCard(),new CricketFunctions.BowlerFiguresComparator());

							for(BowlingCard boc : cricket_matches.getMatch().getInning().get(i-1).getBowlingCard()) {
					...
					if(cricket_matches.getMatch().getMatchResult() != null) {
						if(cricket_matches.getMatch().getMatchResult().toUpperCase().equalsIgnoreCase("DRAWN")){
							print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSumResult " + "MATCH TIED" + ";");
						}
						else if(cricket_matches.getMatch().getMatchResult().split(",")[1].toUpperCase().equalsIgnoreCase("SUPER_OVER")){
							print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSumResult " + "MATCH TIED - " 
						+ cricket_matches.getMatch().getMatchStatus().toUpperCase() + ";");
						}
						else {
							print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSumResult " + cricket_matches.getMatch().getMatchStatus().toUpperCase() + ";");
						}
					}
					else {
						if(cricket_matches.getSetup().getTargetType() == "") {
							print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSumResult " + 
									cricket_matches.getMatch().getMatchStatus().toUpperCase() + ";");
						}
						else if(cricket_matches.getSetup().getTargetType() != null) {
							if(cricket_matches.getSetup().getTargetType().toUpperCase().equalsIgnoreCase("VJD")) {
								print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSumResult " + 
										cricket_matches.getMatch().getMatchStatus().toUpperCase() + " (VJD)" + ";");
							}
							else if(cricket_matches.getSetup().getTargetType().toUpperCase().equalsIgnoreCase("DLS")) {
								print_writer.println("LAYER2*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tSumResult " + 
										cricket_matches.getMatch().getMatchStatus().toUpperCase() + " (DLS)" + ";");
							}
						}
