...
public class EVEREST_PPL_T20 extends Scene{

	public String broadcaster = "EVEREST_PPL_T20";
	public String status;
	public String slashOrDash = "-";
	//public String logo_path_ns = "C:\\\\Images\\\\PPL\\\\Logos\\\\";
	...
	public EVEREST_PPL_T20() {
		super();
	}

	public EVEREST_PPL_T20(String scene_path, String which_Layer) {
			...
			PrintWriter print_writer, List<Scene> scenes, String valueToProcess, List<Statistics> statistics, Configuration config) throws InterruptedException, ParseException, JAXBException, IllegalAccessException, InvocationTargetException, IOException
	{
		...
		case "ANIMATE-IN-SPLIT-DB": case "ANIMATE-FF_GRAPHICS": case "ANIMATE-LT_GRAPHICS": case "ANIMATE-IN-BUG-RESULT": case "ANIMATE-IN-CLIP":
		...
		case "ANIMATE-IN-TEAM_TOURNAMENT":
			...
			case "ANIMATE-IN-BUG_POWERPLAY": case "ANIMATE-IN-BUG_PARTNERSHIP": case "ANIMATE-IN-BUG_HIGHLIGHT": case "ANIMATE-IN-MULTI_PARTNERSHIP":
			case "ANIMATE-IN-FINAL_MATCH_SUMMARY": case "ANIMATE-IN-BUG-RESULT": case "ANIMATE-IN-CLIP":
			...
			case "ANIMATE-IN-BUG-RESULT":
				processAnimation(print_writer, "In", "START", broadcaster);
				which_graphic_on_screen = "BUG_RESULT";
				break;
			case "ANIMATE-IN-CLIP":
				processAnimation(print_writer, "In", "START", broadcaster);
				which_graphic_on_screen = "CLIP";
				break;
...
//				AnimateInGraphics(print_writer, "MULTI-PARTNERSHIP");
			...
			case "ANIMATE-OUT":
				...
				case "SPLIT-DB":
					...
					break;
				case "BUG_RESULT":
					AnimateOutGraphics(print_writer, "BUG_RESULT");
					TimeUnit.SECONDS.sleep(1);
					
					which_graphic_on_screen = "";
					break;
				case "CLIP":
					AnimateOutGraphics(print_writer, "CLIP");
					TimeUnit.SECONDS.sleep(1);
		...
		case "POPULATE-L3-BUG-RESULT": case "POPULATE-CLIP":
...
//					print_writer.println("-1 RENDERER*STAGE SHOW 0.0 \0");
			...
			case "POPULATE-L3-BUG-RESULT":
				populateBugResult(print_writer, valueToProcess.split(",")[0], match, broadcaster);
				break;
			case "POPULATE-CLIP":
				populateClip(print_writer, broadcaster);
				break;
...
//				break;	
	...
	public void AnimateOutGraphics(PrintWriter print_writer, String whichGraphic) throws InterruptedException
	{
		...
		case "FF_GRAPHICS": case "LT_GRAPHICS":	case "TOURNAMENT_BATTING":	case "TOURNAMENT_BOWLING":	case "TOURNAMENT_TEAM":	case "TEAM_TOURNAMENT": case "BUG_RESULT":
		case "CLIP":	
	...
	public void populateBugDismissal(PrintWriter print_writer, String viz_scene, int whichInning, String statsType,
		...
		case "EVEREST_PPL_T20":
			if (match == null) {
								...
								if (bc.getPlayerId() == playerId) {
									...
									if(bc.getHowOut().equalsIgnoreCase(CricketUtil.RUN_OUT)) {
										if(CricketFunctions.isImpactPlayer(match.getEventFile().getEvents(), inn.getInningNumber(), bc.getHowOutFielderId()).equalsIgnoreCase(CricketUtil.YES)) {
											how_out_txt = "run out (impact - " + bc.getHowOutFielder().getTicker_name() + ") "+ bc.getHowOutPartTwo();
										}else {
											if(bc.getWasHowOutFielderSubstitute() != null && bc.getWasHowOutFielderSubstitute().equalsIgnoreCase(CricketUtil.YES)) {
												...
												how_out_txt =  "run out (sub)" ;
											}else {
												how_out_txt = bc.getHowOutText();
											}
										}
									}else if(bc.getHowOut().equalsIgnoreCase(CricketUtil.CAUGHT)) {
										if(CricketFunctions.isImpactPlayer(match.getEventFile().getEvents(), inn.getInningNumber(), bc.getHowOutFielderId()).equalsIgnoreCase(CricketUtil.YES)) {
											how_out_txt = "c (impact - "+bc.getHowOutFielder().getTicker_name()+") "+ bc.getHowOutPartTwo() ;
										}else {
											if(bc.getWasHowOutFielderSubstitute() != null && bc.getWasHowOutFielderSubstitute().equalsIgnoreCase(CricketUtil.YES)){
												how_out_txt = "c "+ "(sub - "+bc.getHowOutFielder().getTicker_name()+") "+bc.getHowOutPartTwo() ;
											}else if(bc.getHowOutPartTwo().equalsIgnoreCase("substitute")){
												how_out_txt ="c "+ "(sub)";
											}else{
												how_out_txt = bc.getHowOutText() ;
				...
				print_writer.println("LAYER1*EVEREST*STAGE*DIRECTOR*In STOP;");
				print_writer.println("LAYER1*EVEREST*STAGE*DIRECTOR*Out STOP;");
				print_writer.println("LAYER1*EVEREST*STAGE*DIRECTOR*In SHOW 100.0;");
				print_writer.println("LAYER1*EVEREST*STAGE*DIRECTOR*Out SHOW 0.0;");
				print_writer.println("LAYER1*EVEREST*GLOBAL SNAPSHOT_PATH C:/Temp/Preview.png;");
				print_writer.println("LAYER1*EVEREST*GLOBAL SNAPSHOT 1920 1080;");
				TimeUnit.SECONDS.sleep(1);
...
//													+ "Over" + CricketFunctions.Plural(Integer.valueOf(CricketFunctions.OverBalls(boc.getOvers(), boc.getBalls()))) + ";");
	...
	public void populateBugResult(PrintWriter print_writer, String viz_scene, MatchAllData match,String session_selected_broadcaster) throws InterruptedException {
		switch (session_selected_broadcaster.toUpperCase()) {
		case "EVEREST_PPL_T20":
			if (match == null) {
				this.status = "ERROR: Match is null";
			} else if (match.getMatch().getInning() == null) {
				this.status = "ERROR: Result's inning is null";
			} else {
				
//				if(match.getMatch().getMatchStatus() != null && !match.getMatch().getMatchStatus().isEmpty()) {
//					if(match.getMatch().getMatchStatus().contains("win")) {
//						print_writer.println("LAYER1*EVEREST*TREEVIEW*Main*FUNCTION*TAG_CONTROL SET tPlayerName01 " + 
//								CricketFunctions.generateMatchSummaryStatus(2, match, CricketUtil.FULL, 
...
//				}
	...
	public void populateClip(PrintWriter print_writer,String session_selected_broadcaster) throws InterruptedException {
		switch (session_selected_broadcaster.toUpperCase()) {
		case "EVEREST_PPL_T20":
